...
;(function(factory, global) {
	...
	Promise.prototype = {
		...
		then: function(onResolve, onReject) {
			...
			this._doneCallbacks.push(
				makeCallback(promise, onResolve, 'resolve')
			)
	...
	Promise.race = function(iterable) {
		if (!iterable || !iterable.hasOwnProperty('length')) {
			throw new TypeError(
				'TypeError: Parameter `iterable` must be a iterable object'
			)
		}
	...
	Promise.all = function(iterable) {
		...
		for (var i = 0; i < length; i++) {
			...
			iterate.then(
				makeAllCallback(iterate, i, 'resolve'),
				makeAllCallback(iterate, i, 'reject')
			)
		}
			...
			if (typeof callback === 'function') {
				...
				if (x === promise) {
					var reason = new TypeError(
						'TypeError: The return value could not be same with the promise'
					)
					reject(promise, reason)
				}
								...
								try {
									then.call(
										...
										function(y) {
											...
											if (y === x) {
												throw new TypeError(
													'TypeError: The return value could not be same with the previous thenable object'
												)
											}
